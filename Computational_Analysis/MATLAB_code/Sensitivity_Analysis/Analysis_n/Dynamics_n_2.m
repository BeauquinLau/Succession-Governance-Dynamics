...
for i = 1:length(n_values)
    ...
    J_E = calculate_jacobian_conflict(p_val, q_val, n);
    ...
    [~, Y_uni_pos] = ode45(@(t, y) replicator_dynamics_conflict(t, y, n), [0 5000], init_unstable_pos, options);
    [~, Y_uni_neg] = ode45(@(t, y) replicator_dynamics_conflict(t, y, n), [0 5000], init_unstable_neg, options);
    ...
    [~, Y_sta_pos] = ode45(@(t, y) replicator_dynamics_conflict(t, y, n), [0 -5000], init_stable_pos, options);
    [~, Y_sta_neg] = ode45(@(t, y) replicator_dynamics_conflict(t, y, n), [0 -5000], init_stable_neg, options);
    
    % Plot the primary trajectory and store its handle for the legend
    h = plot(Y_uni_pos(:, 1), Y_uni_pos(:, 2), '--', 'Color', colors(i, :), 'LineWidth', 2, 'DisplayName', sprintf('$n = %.3f$', n));
    legend_handles(i) = h;
...
plot(0.2, 0.7, 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(0.21, 0.702, 'D', 'FontSize', 13, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
plot(0.5, 0.4, 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(0.5-0.03, 0.4-0.05, 'E', 'FontSize', 13, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
plot(0.7, 0.5, 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(0.702, 0.502, 'F', 'FontSize', 13, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
...
function dydt = replicator_dynamics_conflict(~, y, n)
    % Define system parameters for the "Conflictual Case"
    gamma = 0.5;
    omega = 0.2;
    phi_FS = 0.05;
...
function J = calculate_jacobian_conflict(p, q, n)
