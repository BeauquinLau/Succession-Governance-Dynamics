...
for i = 1:length(n_values)
    ...
    % Solve the differential equation from the single starting point 'C'
...
% --- Plot Annotations ---
% Plot the initial starting point 'C'
plot(initial_condition(1), initial_condition(2), 'ks', 'MarkerSize', 8, 'MarkerFaceColor', 'y');
text(initial_condition(1), initial_condition(2) - 0.04, '$C$', 'FontSize', 14, 'HorizontalAlignment', 'center', 'Interpreter','latex');
...
title('Trajectory Sensitivity to Monitoring Intensity $n$ from Point $C$','Interpreter','latex');
...
ax = gca;
ax.LineWidth = 1.2;
outputFileName = 'Sensitivity_C.pdf';
