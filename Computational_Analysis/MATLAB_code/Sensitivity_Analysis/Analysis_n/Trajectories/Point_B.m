%%% === SCRIPT SETUP FOR TRAJECTORY SENSITIVITY ANALYSIS === %%%
...
for i = 1:length(n_values)
    ...
    % Solve the differential equation from the single starting point 'B'
...
% --- Plot Annotations ---
% Plot the initial starting point 'B'
plot(initial_condition(1), initial_condition(2), 'ks', 'MarkerSize', 8, 'MarkerFaceColor', 'y');
text(initial_condition(1), initial_condition(2) - 0.04, '$B$', 'FontSize', 14, 'HorizontalAlignment', 'center', 'Interpreter', 'latex');

% Plot the pure-strategy equilibria with correct stability colors
...
title('Trajectory Sensitivity to Monitoring Intensity $n$ from Point $B$','Interpreter','latex');
...
ax = gca;
ax.LineWidth = 1.2;
outputFileName = 'Sensitivity_B.pdf';
...
function dydt = replicator_dynamics(~, y, n)
    ...
    % Replicator dynamic equation for q
