...
for i = 1:length(n_values)
    ...
    % Solve the differential equation from the single starting point 'F'
...
% --- Plot Annotations ---
% Plot the initial starting point 'F'
plot(initial_condition(1), initial_condition(2), 'ks', 'MarkerSize', 8, 'MarkerFaceColor', 'y');
text(initial_condition(1), initial_condition(2) + 0.04, '$F$', 'FontSize', 14, 'HorizontalAlignment', 'center','Interpreter','latex');
...
title('Trajectory Sensitivity to Conflict Intensity $n$ from Point $F$','Interpreter','latex');
...
ax = gca;
ax.LineWidth = 1.2;
outputFileName = 'Sensitivity_F.pdf';
