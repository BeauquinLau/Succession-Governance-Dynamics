...
% These parameters are for the 'F' simulation.
gamma = 0.5;
omega = 0.2;
phi_FS = 0.05;
phi_NPM = 0.1;
...
initial_condition = [0.7 0.5]; % This is the new starting point 'F'
...
%%% --- Plot Annotations --- %%%
% Plot the initial starting point 'F' with a marker and a label
plot(initial_condition(1), initial_condition(2), 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(initial_condition(1) + 0.01, initial_condition(2) + 0.02, 'F', 'FontSize', 13, ...
...
outputFileName = 'F.pdf'; % Changed filename to reflect the new plot
