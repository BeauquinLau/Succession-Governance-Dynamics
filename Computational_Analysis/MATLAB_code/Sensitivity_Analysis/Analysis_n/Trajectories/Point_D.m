...
for i = 1:length(n_values)
    ...
    % Solve the differential equation from the single starting point 'D'
...
% --- Plot Annotations ---
% Plot the initial starting point 'D'
plot(initial_condition(1), initial_condition(2), 'ks', 'MarkerSize', 8, 'MarkerFaceColor', 'y');
text(initial_condition(1), initial_condition(2) - 0.04, '$D$', 'FontSize', 14, 'HorizontalAlignment', 'center','Interpreter','latex');

% Plot the pure-strategy equilibria with UPDATED stability colors
...
title('Trajectory Sensitivity to Conflict Intensity $n$ from Point $D$','Interpreter','latex');
...
ax = gca;
ax.LineWidth = 1.2;
outputFileName = 'Sensitivity_D.pdf';
