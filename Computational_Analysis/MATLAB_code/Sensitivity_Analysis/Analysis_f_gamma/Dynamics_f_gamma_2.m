...
for i = 1:length(f_gamma_values)
    ...
    [~, Y_uni_pos] = ode45(@(t, y) replicator_dynamics_conflict(t, y, f_gamma), [0 5000], init_unstable_pos, options);
    [~, Y_uni_neg] = ode45(@(t, y) replicator_dynamics_conflict(t, y, f_gamma), [0 5000], init_unstable_neg, options);
    ...
    [~, Y_sta_pos] = ode45(@(t, y) replicator_dynamics_conflict(t, y, f_gamma), [0 -5000], init_stable_pos, options);
    [~, Y_sta_neg] = ode45(@(t, y) replicator_dynamics_conflict(t, y, f_gamma), [0 -5000], init_stable_neg, options);
    
    % Plot the primary trajectory and store its handle for the legend
    h = plot(Y_uni_pos(:, 1), Y_uni_pos(:, 2), '--', 'Color', colors(i, :), 'LineWidth', 2, 'DisplayName', sprintf('$f_{\\gamma} = %.3f$', f_gamma));
    legend_handles(i) = h;
...
% Plot representative initial points for analysis (P, Q, R)
plot(0.2, 0.8, 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(0.21, 0.802, 'P', 'FontSize', 13, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
plot(0.5, 0.5, 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(0.51, 0.5-0.05, 'Q', 'FontSize', 13, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
plot(0.8, 0.2, 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(0.802, 0.202, 'R', 'FontSize', 13, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
...
lgd.Title.String = '\hspace{0.5em}Varying Parameter $f_{\gamma}$\hspace{0.5em}';
...
function dydt = replicator_dynamics_conflict(~, y, f_gamma)
    % Define system parameters for the "Conflictual Case"
    gamma = 0.5;
    omega = 0.2;
    phi_FS = 0.05;
