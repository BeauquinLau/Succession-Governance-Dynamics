...
% These are the new fixed parameters for the 'P' simulation.
gamma = 0.5;
omega = 0.2;
phi_FS = 0.05;
phi_NPM = 0.1;
...
initial_condition = [0.2 0.8]; % This is the new starting point 'P'
...
%%% --- Plot Annotations --- %%%
% Plot the initial starting point 'P' with a marker and a label
plot(initial_condition(1), initial_condition(2), 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(initial_condition(1) + 0.02, initial_condition(2) - 0.03, 'P', 'FontSize', 13, ...
...
outputFileName = 'P.pdf'; % Changed filename to reflect the new plot
