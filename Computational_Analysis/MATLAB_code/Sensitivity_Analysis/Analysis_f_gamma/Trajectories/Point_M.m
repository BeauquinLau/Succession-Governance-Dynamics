%%% === SCRIPT SETUP FOR TRAJECTORY SENSITIVITY FROM POINT M (COOPERATIVE CASE) === %%%
...
initial_condition = [0.3, 0.3]; % Starting point M
...
for i = 1:length(f_gamma_values)
    ...
    ode_function = @(t, y) replicator_dynamics_M(t, y, f_gamma);
    
    % Solve the differential equation from the single starting point 'M'
...
% --- Plot Annotations ---
% Plot the initial starting point 'M'
plot(initial_condition(1), initial_condition(2), 'ks', 'MarkerSize', 8, 'MarkerFaceColor', 'y');
text(initial_condition(1), initial_condition(2) - 0.04, '$M$', 'FontSize', 14, 'HorizontalAlignment', 'center','Interpreter','latex');
...
title('Trajectory Sensitivity from Point $M$ to Varying Affective Benefit $f_{\gamma}$','Interpreter','latex');
