%%% === SCRIPT SETUP FOR TRAJECTORY SENSITIVITY FROM POINT P (CONFLICTUAL CASE) === %%%
...
initial_condition = [0.2, 0.8]; % Starting point P
...
for i = 1:length(f_gamma_values)
    ...
    ode_function = @(t, y) replicator_dynamics_P_conflict(t, y, f_gamma);
    
    % Solve the differential equation from the single starting point 'P'
...
% --- Plot Annotations ---
% Plot the initial starting point 'P'
plot(initial_condition(1), initial_condition(2), 'ks', 'MarkerSize', 8, 'MarkerFaceColor', 'y');
text(initial_condition(1), initial_condition(2) - 0.04, '$P$', 'FontSize', 14, 'HorizontalAlignment', 'center','Interpreter','latex');
...
title('Trajectory Sensitivity from Point $P$ to Varying $f_{\gamma}$ (Conflictual Case)','Interpreter','latex');
