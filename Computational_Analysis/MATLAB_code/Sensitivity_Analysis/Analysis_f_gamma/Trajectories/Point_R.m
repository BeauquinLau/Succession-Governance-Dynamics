%%% === SCRIPT SETUP FOR TRAJECTORY SENSITIVITY FROM POINT R (CONFLICTUAL CASE) === %%%
...
initial_condition = [0.8, 0.2]; % Starting point R
...
for i = 1:length(f_gamma_values)
    ...
    ode_function = @(t, y) replicator_dynamics_R_conflict(t, y, f_gamma);
    
    % Solve the differential equation from the single starting point 'R'
...
% --- Plot Annotations ---
% Plot the initial starting point 'R'
plot(initial_condition(1), initial_condition(2), 'ks', 'MarkerSize', 8, 'MarkerFaceColor', 'y');
text(initial_condition(1), initial_condition(2) + 0.04, '$R$', 'FontSize', 14, 'HorizontalAlignment', 'center','Interpreter','latex');
...
title('Trajectory Sensitivity from Point $R$ to Varying $f_{\gamma}$ (Conflictual Case)','Interpreter','latex');
