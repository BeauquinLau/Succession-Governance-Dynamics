%%% === SCRIPT SETUP FOR TRAJECTORY SENSITIVITY FROM POINT O (COOPERATIVE CASE) === %%%
...
initial_condition = [0.9, 0.5]; % Starting point O
...
for i = 1:length(f_gamma_values)
    ...
    ode_function = @(t, y) replicator_dynamics_O(t, y, f_gamma);
    
    % Solve the differential equation from the single starting point 'O'
...
% --- Plot Annotations ---
% Plot the initial starting point 'O'
plot(initial_condition(1), initial_condition(2), 'ks', 'MarkerSize', 8, 'MarkerFaceColor', 'y');
text(initial_condition(1), initial_condition(2) - 0.04, '$O$', 'FontSize', 14, 'HorizontalAlignment', 'center','Interpreter','latex');
...
title('Trajectory Sensitivity from Point $O$ to Varying Affective Benefit $f_{\gamma}$','Interpreter','latex');
