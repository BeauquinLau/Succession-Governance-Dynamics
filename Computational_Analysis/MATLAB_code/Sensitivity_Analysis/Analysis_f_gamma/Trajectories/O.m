...
% These are the fixed parameters for the 'O' simulation.
gamma = 0.5;
omega = 0.28;
phi_FS = 0.2;
phi_NPM = 0.25;
...
initial_condition = [0.9 0.4]; % This is the new starting point 'O'
...
for i = 1:length(f_gamma_values)
    ...
    legend_label = sprintf('$f_{\\gamma} = %.0f$', f_gamma);
...
%%% --- Plot Annotations --- %%%
% Plot the initial starting point 'O' with a marker and a label
plot(initial_condition(1), initial_condition(2), 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(initial_condition(1) - 0.04, initial_condition(2) - 0.02, 'O', 'FontSize', 13, ...
...
outputFileName = 'O.pdf'; % Changed filename to reflect the new plot
