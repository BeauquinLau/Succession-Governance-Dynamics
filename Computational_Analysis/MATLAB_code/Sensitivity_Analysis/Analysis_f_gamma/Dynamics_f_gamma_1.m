...
for i = 1:length(f_gamma_values)
    ...
    [~, Y_uni_pos] = ode45(@(t, y) replicator_dynamics(t, y, f_gamma), [0 1000], init_unstable_pos, options);
    [~, Y_uni_neg] = ode45(@(t, y) replicator_dynamics(t, y, f_gamma), [0 1000], init_unstable_neg, options);
    ...
    [~, Y_sta_pos] = ode45(@(t, y) replicator_dynamics(t, y, f_gamma), [0 -1000], init_stable_pos, options);
    [~, Y_sta_neg] = ode45(@(t, y) replicator_dynamics(t, y, f_gamma), [0 -1000], init_stable_neg, options);
    
    % Plot the primary trajectory and store its handle for the legend
    h = plot(Y_uni_pos(:,1), Y_uni_pos(:,2), '--', 'Color', colors(i,:), 'LineWidth', 2, 'DisplayName', sprintf('$f_{\\gamma} = %.0f$', f_gamma));
    legend_handles(i) = h;
...
% Plot representative initial points for analysis (M, N, O)
plot(0.3, 0.3, 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(0.302, 0.302, 'M', 'FontSize', 13, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
plot(0.5, 0.6, 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(0.505, 0.6+0.02, 'N', 'FontSize', 13, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
plot(0.9, 0.4, 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(0.902, 0.402, 'O', 'FontSize', 13, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
