%%% === SCRIPT SETUP FOR TRAJECTORY SENSITIVITY FROM POINT K === %%%
...
initial_condition = [0.6, 0.5]; % Starting point K
...
for i = 1:length(phi_NPM_values)
    ...
    ode_function = @(t, y) replicator_dynamics_K(t, y, phi_NPM);
    
    % Solve the differential equation from the single starting point 'K'
...
% --- Plot Annotations ---
% Plot the initial starting point 'K'
plot(initial_condition(1), initial_condition(2), 'ks', 'MarkerSize', 8, 'MarkerFaceColor', 'y');
text(initial_condition(1) + 0.01, initial_condition(2) - 0.04, '$K$', 'FontSize', 14, 'HorizontalAlignment', 'center','Interpreter','latex');
...
title('Trajectory Sensitivity to Agency Cost $\phi_{NPM}$ from Point $K$','Interpreter','latex');
