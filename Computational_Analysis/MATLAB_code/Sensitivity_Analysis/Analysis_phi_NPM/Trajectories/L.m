...
% These are the fixed parameters for the 'L' simulation.
gamma = 0.5;
omega = 0.2;
phi_FS = 0.05;
m = 0.05;
...
initial_condition = [0.9 0.4]; % This is the new starting point 'L'
...
%%% --- Plot Annotations --- %%%
% Plot the initial starting point 'L' with a marker and a label
plot(initial_condition(1), initial_condition(2), 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(initial_condition(1) + 0.01, initial_condition(2) + 0.01, 'L', 'FontSize', 13, ...
...
outputFileName = 'L.pdf'; % Changed filename to reflect the new plot
