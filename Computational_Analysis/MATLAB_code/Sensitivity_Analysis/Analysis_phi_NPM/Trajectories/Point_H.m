%%% === SCRIPT SETUP FOR TRAJECTORY SENSITIVITY ANALYSIS FROM POINT H === %%%
...
initial_condition = [0.7, 0.6]; % This is the starting point 'H'
...
for i = 1:length(phi_NPM_values)
    ...
    % Solve the differential equation from the single starting point 'H'
...
% --- Plot Annotations ---
% Plot the initial starting point 'H'
plot(initial_condition(1), initial_condition(2), 'ks', 'MarkerSize', 8, 'MarkerFaceColor', 'y');
text(initial_condition(1) - 0.02, initial_condition(2) + 0.04, '$H$', 'FontSize', 14, 'HorizontalAlignment', 'center', 'Interpreter', 'latex');
...
title('Trajectory Sensitivity to Agency Cost $\phi_{NPM}$ from Point $H$','Interpreter','latex');
...
ax = gca;
ax.LineWidth = 1.2;
outputFileName = 'Sensitivity_H.pdf';
