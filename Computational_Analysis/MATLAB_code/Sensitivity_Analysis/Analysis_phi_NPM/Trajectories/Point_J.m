%%% === SCRIPT SETUP FOR TRAJECTORY SENSITIVITY FROM POINT J === %%%
...
initial_condition = [0.2, 0.7]; % Starting point J
...
for i = 1:length(phi_NPM_values)
    ...
    ode_function = @(t, y) replicator_dynamics_J(t, y, phi_NPM);
    
    % Solve the differential equation from the single starting point 'J'
...
% --- Plot Annotations ---
% Plot the initial starting point 'J'
plot(initial_condition(1), initial_condition(2), 'ks', 'MarkerSize', 8, 'MarkerFaceColor', 'y');
text(initial_condition(1), initial_condition(2) - 0.04, '$J$', 'FontSize', 14, 'HorizontalAlignment', 'center','Interpreter','latex');

% Plot the pure-strategy equilibria with stability colors for the conflictual case
...
title('Trajectory Sensitivity to Agency Cost $\phi_{NPM}$ from Point $J$','Interpreter','latex');
