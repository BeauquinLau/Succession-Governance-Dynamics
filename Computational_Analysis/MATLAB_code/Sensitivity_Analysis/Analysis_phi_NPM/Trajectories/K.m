...
% These are the fixed parameters for the 'K' simulation.
gamma = 0.5;
omega = 0.2;
phi_FS = 0.05;
m = 0.05;
...
initial_condition = [0.6 0.5]; % This is the new starting point 'K'
...
%%% --- Plot Annotations --- %%%
% Plot the initial starting point 'K' with a marker and a label
plot(initial_condition(1), initial_condition(2), 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(initial_condition(1) + 0.02, initial_condition(2) - 0.03, 'K', 'FontSize', 13, ...
...
outputFileName = 'K.pdf'; % Changed filename to reflect the new plot
