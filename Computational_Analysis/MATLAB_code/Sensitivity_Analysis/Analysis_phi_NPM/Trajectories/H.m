...
% These are the fixed parameters for the 'H' simulation.
gamma = 0.5;
omega = 0.28;
phi_FS = 0.2;
m = 0.1;
n = 0.05; % Note: 'n' is now a fixed parameter.
V = 100;
W = 90;
C = 50;
K_m = 20;
...
initial_condition = [0.4 0.6]; % This is the new starting point 'H'
...
%%% --- Plot Annotations --- %%%
% Plot the initial starting point 'H' with a marker and a label
plot(initial_condition(1), initial_condition(2), 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(initial_condition(1) - 0.04, initial_condition(2) + 0.02, 'H', 'FontSize', 13, ...
...
outputFileName = 'H.pdf'; % Changed filename to reflect the new plot
...
function dydt = replicator_dynamics(~, y, phi_NPM)
    ...
    phi_FS = 0.2;
    m = 0.1;
    n = 0.05; % 'n' is fixed in this simulation
    V = 100;
    W = 90;
    C = 50;
    K_m = 20;
