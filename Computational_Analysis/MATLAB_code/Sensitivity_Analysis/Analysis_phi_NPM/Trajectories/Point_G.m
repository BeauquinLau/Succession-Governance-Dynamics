%%% === SCRIPT SETUP FOR TRAJECTORY SENSITIVITY ANALYSIS FROM POINT G === %%%
...
initial_condition = [0.1, 0.4]; % This is the starting point 'G'
...
for i = 1:length(phi_NPM_values)
    ...
    % Solve the differential equation from the single starting point 'G'
...
% --- Plot Annotations ---
% Plot the initial starting point 'G'
plot(initial_condition(1), initial_condition(2), 'ks', 'MarkerSize', 8, 'MarkerFaceColor', 'y');
text(initial_condition(1), initial_condition(2) + 0.04, '$G$', 'FontSize', 14, 'HorizontalAlignment', 'center', 'Interpreter', 'latex');
...
title('Trajectory Sensitivity to Agency Cost $\phi_{NPM}$ from Point $G$','Interpreter','latex');
...
ax = gca;
ax.LineWidth = 1.2;
outputFileName = 'Sensitivity_G.pdf';
