%%% === SCRIPT SETUP === %%%
% --- Define System Parameters for the "Conflictual Case" ---
gamma = 0.5;
omega = 0.2;
phi_FS = 0.05;
m = 0.05;
n = -0.3;
V = 1;
W = 1.2;
C = 0.25;
K_m = 0.05;
f_gamma = 0.355;

% --- Simulation Configuration ---
% Define the range of the parameter 'phi_NPM' to be analyzed
phi_NPM_values = 0.06:0.01:0.16;

% Pre-allocate arrays to store the results for the saddle point coordinates
p_star = zeros(size(phi_NPM_values));
q_star = zeros(size(phi_NPM_values));

% --- Calculate Saddle Point Coordinates for each 'phi_NPM' ---
for i = 1:length(phi_NPM_values)
    phi_NPM = phi_NPM_values(i);
    
    % Calculate p*
    numerator_p = C - K_m - (gamma * (1 - omega) + (1 - gamma) * (phi_FS - n)) * V + ...
        gamma * (1 - omega - phi_NPM + m + n) * W;
    denominator_p = (1 - (gamma * (1 - omega) + (1 - gamma) * (phi_FS - n))) * V + ...
        (gamma * (1 - omega - phi_NPM + m + n) - (1 - omega - phi_NPM + m)) * W;
    p_star(i) = numerator_p / denominator_p;
    
    % Calculate q* (which is constant for a fixed n)
    numerator_q = n * W + f_gamma;
    denominator_q = (phi_FS - n) * V + n * W;
    q_star(i) = numerator_q / denominator_q;
end

% --- Plotting Setup ---
eps = 1e-5; % Perturbation size for numerical stability
options = odeset('RelTol', 1e-6, 'AbsTol', 1e-9);

% Initialize figure and colormap
figure;
hold on;
colors = jet(length(phi_NPM_values));
legend_handles = gobjects(length(phi_NPM_values), 1); % Pre-allocate for legend handles

% --- Calculate and Plot Manifolds for Each 'phi_NPM' ---
for i = 1:length(phi_NPM_values)
    phi_NPM = phi_NPM_values(i);
    p_val = p_star(i);
    q_val = q_star(i);
    
    % Calculate the Jacobian matrix numerically at the saddle point
    J_E = calculate_jacobian_phi_conflict(p_val, q_val, phi_NPM);
    
    % Eigenvalue analysis to find stable and unstable directions
    [V, D] = eig(J_E);
    lambda = diag(D);
    [~, idx_pos] = max(real(lambda));
    [~, idx_neg] = min(real(lambda));
    v_unstable = V(:, idx_pos);
    v_stable = V(:, idx_neg);
    
    % Integrate along the unstable manifold
    init_unstable_pos = [p_val; q_val] + eps * v_unstable;
    init_unstable_neg = [p_val; q_val] - eps * v_unstable;
    [~, Y_uni_pos] = ode45(@(t, y) replicator_dynamics_conflict(t, y, phi_NPM), [0 5000], init_unstable_pos, options);
    [~, Y_uni_neg] = ode45(@(t, y) replicator_dynamics_conflict(t, y, phi_NPM), [0 5000], init_unstable_neg, options);
    
    % Integrate along the stable manifold (backward in time)
    init_stable_pos = [p_val; q_val] + eps * v_stable;
    init_stable_neg = [p_val; q_val] - eps * v_stable;
    [~, Y_sta_pos] = ode45(@(t, y) replicator_dynamics_conflict(t, y, phi_NPM), [0 -5000], init_stable_pos, options);
    [~, Y_sta_neg] = ode45(@(t, y) replicator_dynamics_conflict(t, y, phi_NPM), [0 -5000], init_stable_neg, options);
    
    % Plot the primary trajectory and store its handle for the legend
    h = plot(Y_uni_pos(:, 1), Y_uni_pos(:, 2), '--', 'Color', colors(i, :), 'LineWidth', 2, 'DisplayName', sprintf('$\\phi_{NPM} = %.2f$', phi_NPM));
    legend_handles(i) = h;
    
    % Plot the other three manifold arms without legend entries
    plot(Y_uni_neg(:, 1), Y_uni_neg(:, 2), '--', 'Color', colors(i, :), 'LineWidth', 2, 'HandleVisibility', 'off');
    plot(Y_sta_pos(:, 1), Y_sta_pos(:, 2), '--', 'Color', colors(i, :), 'LineWidth', 2, 'HandleVisibility', 'off');
    plot(Y_sta_neg(:, 1), Y_sta_neg(:, 2), '--', 'Color', colors(i, :), 'LineWidth', 2, 'HandleVisibility', 'off');
    
    % Plot the saddle point marker
    plot(p_val, q_val, 'o', 'MarkerFaceColor', colors(i, :), 'MarkerEdgeColor', 'k', 'HandleVisibility', 'off');
end

% --- Plot Fixed Points and Annotations ---
% Plot the pure-strategy equilibria (E1-E4)
plot(0, 0, 'ko', 'MarkerSize', 5, 'MarkerFaceColor', 'g');
text(0.01, 0-0.05, 'E_1', 'FontSize', 12, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
plot(0, 1, 'ko', 'MarkerSize', 5, 'MarkerFaceColor', 'g');
text(0, 1, 'E_2', 'FontSize', 12, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
plot(1, 0, 'ko', 'MarkerSize', 5, 'MarkerFaceColor', 'g');
text(1 + 0.02, 0, 'E_3', 'FontSize', 12, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
plot(1, 1, 'ko', 'MarkerSize', 5, 'MarkerFaceColor', 'g');
text(1, 1, 'E_4', 'FontSize', 12, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');

% Plot representative initial points for analysis (J, K, L)
plot(0.2, 0.5, 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(0.21, 0.502, 'J', 'FontSize', 13, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
plot(0.6, 0.5, 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(0.61, 0.5-0.05, 'K', 'FontSize', 13, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
plot(0.9, 0.4, 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(0.902, 0.402, 'L', 'FontSize', 13, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');

% --- Final Figure Formatting ---
axis([0 1 0 1]);
axis('square');
xlabel('p');
ylabel('q');
grid on;
box on;
xticks(0:0.1:1);
yticks(0:0.1:1);
hold off;

% --- Create, Format, and Add Padding to Legend ---
% The Definitive Fix: Artificially lengthen the longest entry with invisible space
% to trick the layout engine into creating a wider box.
last_handle = legend_handles(end);
last_handle.DisplayName = [last_handle.DisplayName, '\hspace{1.5em}']; % Adjust the em value as needed

% Create the legend. It will now be wide enough.
lgd = legend(legend_handles, 'Interpreter', 'latex', 'AutoUpdate', 'off');

% Set the single-line, padded title, which will now fit perfectly.
lgd.Title.String = '\hspace{0.5em}Varying Parameter $\phi_{NPM}$\hspace{0.5em}';
lgd.Title.Interpreter = 'latex';
set(lgd, 'Location', 'northeastoutside');

% Adjust figure window size for better layout with legend
set(gcf, 'Position', [100 100 800 600]);

% --- Save Figure ---
ax = gca;
ax.LineWidth = 1; % Set axis line width for publication quality

% Define the output filename
outputFileName = 'Dynamics_phi_NPM_2.pdf';

% Save the figure using exportgraphics, which tightly crops the output
exportgraphics(ax, outputFileName, 'ContentType', 'vector');
disp(['Figure saved to: ' fullfile(pwd, outputFileName)]);


%%% --- LOCAL FUNCTIONS --- %%%

function dydt = replicator_dynamics_conflict(~, y, phi_NPM)
    % Define system parameters for the "Conflictual Case"
    gamma = 0.5;
    omega = 0.2;
    phi_FS = 0.05;
    m = 0.05;
    n = -0.3;
    V = 1;
    W = 1.2;
    C = 0.25;
    K_m = 0.05;
    f_gamma = 0.355;
    
    p = y(1);
    q = y(2);
    
    dydt = zeros(2,1);

    % Replicator dynamics equation for p
    dydt(1) = p * (1 - p) * (q * (phi_FS - n) * V - (1 - q) * n * W - f_gamma);
    
    % Replicator dynamics equation for q
    term1 = (p + (1 - p) * (gamma * (1 - omega) + (1 - gamma) * (phi_FS - n))) * V;
    term2 = ((p + (1 - p) * gamma) * (1 - omega - phi_NPM + m) + (1 - p) * gamma * n) * W;
    dydt(2) = q * (1 - q) * (term1 - C - term2 + K_m);
end

function J = calculate_jacobian_phi_conflict(p, q, phi_NPM)
    % System parameters (must be identical to those in the ODE function)
    gamma = 0.5;
    omega = 0.2;
    phi_FS = 0.05;
    m = 0.05;
    n = -0.3;
    V = 1;
    W = 1.2;
    C = 0.25;
    K_m = 0.05;
    f_gamma = 0.355;

    % Analytically derived partial derivatives
    
    % Partial derivatives of the first ODE (dpdt)
    J11 = (1 - 2*p) * (q*(phi_FS - n)*V - (1 - q)*n*W - f_gamma);
    J12 = p*(1 - p) * ((phi_FS - n)*V + n*W);
    
    % Partial derivatives of the second ODE (dqdt)
    d_term1_dp = (1 - (gamma*(1 - omega) + (1 - gamma)*(phi_FS - n)))*V;
    d_term2_dp = ((1 - gamma)*(1 - omega - phi_NPM + m) - gamma*n)*W;
    J21 = q*(1 - q) * (d_term1_dp - d_term2_dp);
    
    term1 = (p + (1 - p)*(gamma*(1 - omega) + (1 - gamma)*(phi_FS - n)))*V;
    term2 = ( (p + (1 - p)*gamma)*(1 - omega - phi_NPM + m) + (1 - p)*gamma*n )*W;
    J22 = (1 - 2*q) * (term1 - C - term2 + K_m);
    
    % Assemble the Jacobian matrix
    J = [J11, J12; J21, J22];
end