...
for i = 1:length(phi_NPM_values)
    ...
    J_E = calculate_jacobian_phi(p_val, q_val, phi_NPM);
    ...
    [~, Y_uni_pos] = ode45(@(t, y) replicator_dynamics(t, y, phi_NPM), [0 1000], init_unstable_pos, options);
    [~, Y_uni_neg] = ode45(@(t, y) replicator_dynamics(t, y, phi_NPM), [0 1000], init_unstable_neg, options);
    ...
    [~, Y_sta_pos] = ode45(@(t, y) replicator_dynamics(t, y, phi_NPM), [0 -1000], init_stable_pos, options);
    [~, Y_sta_neg] = ode45(@(t, y) replicator_dynamics(t, y, phi_NPM), [0 -1000], init_stable_neg, options);
...
% (Plotting code for E1-E4 and G, H, I remains the same)
...
plot(0.1, 0.4, 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(0.102, 0.402, 'G', 'FontSize', 13, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
plot(0.4, 0.6, 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(0.4-0.03, 0.6-0.05, 'H', 'FontSize', 13, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
plot(0.9, 0.4, 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
text(0.902, 0.402, 'I', 'FontSize', 13, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
...
function J = calculate_jacobian_phi(p, q, phi_NPM)
